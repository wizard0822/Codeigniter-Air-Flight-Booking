...
class Inbound extends UI_Controller {
  ...
  public function index(){
    ...
    if($this->input->post('submit'))
    {
      ...
      $data_field['book_by_name'] ="Inbound";
      ...
      $this->session->set_userdata('pickup_minute',$this->input->post('pickup_minute'));
    ...
    $this->get_include();
    $this->load->view($this->viewDir.'inbound/step1',$this->data);
  }
  ...
  public function fetch_hour_minute(){
    ...
    if($selected_date==$current_date){
       ...
       public function step2(){
        if($this->session->userdata('last_id'))
        {
          ...
          if($this->input->post('submit'))
          {
           ...
           $this->session->set_userdata('desAddress',$this->input->post('desAddress'));
           $this->session->set_userdata('desLattitude',$this->input->post('desLattitude'));
           $this->session->set_userdata('desLognitude',$this->input->post('desLognitude'));
      ...
      $this->get_include();
      $this->load->view($this->viewDir.'inbound/step2',$this->data);
    }
      ...
      if($this->session->userdata('last_id'))
      {
        ...
        if($this->input->post('submit'))
        {
          ...
          if(!empty($edit))
          {
            ...
            {
             redirect('inbound/step4');
           }
           else
           {
            $url =base_url()."inbound/step4";
                        //t($url,1);
    ...
    $this->get_include();
    $this->load->view($this->viewDir.'inbound/step3',$this->data);
  }
  ...
  public function step4(){
    if($this->session->userdata('last_id'))
    {
      ...
      $cond1="AND id_airport='$airport_id' AND id_vehicletype='$id_vehicletype' AND flight_pickup=1 AND id_zone='$zone_id' AND status=1";
      ...
      $customer_price = round(($price_details[0]['customer_price'] + $vehicle_customer_price),2);
      $driver_price = round(($price_details[0]['driver_price'] + $vehicle_driver_price),2);
  ...
  $this->data['userDetails'] = $userDetails;
  $this->get_include();
  $this->load->view($this->viewDir.'inbound/step4',$this->data);
}
public function step5()
{
  ...
  $this->get_include();
  $this->load->view($this->viewDir.'inbound/step5',$this->data);
}
public function step6()
{
  if($this->session->userdata('last_id'))
  {
    ...
    if($this->input->post("submit"))
    {
      ...
      $data_field['tips']= $tips;
                // $data_field['customer_price']= $customer_price;
      $edit = $this->common_model->edit_cond('booking_tbl', $data_field, $cond1);
      $this->session->set_userdata('tips',$tips);
      redirect('inbound/step7');
    }
  }
  else
  {
  ...
  $this->get_include();
  $this->load->view($this->viewDir.'inbound/step6',$this->data);
}
  ...
  if($this->session->userdata('last_id'))
  {
    ...
    if(!empty($id))
    {
      ...
      $date = $datetime->format('c');
                //$date = date('Y-m-d\TH:i:sO', strtotime($details[0]['pickup_date']));
                // t($date);
    }
    else
    {
   ...
   if($this->input->post("submit"))
   {
    ...
    $tips = $details[0]['tips'];
    $passenger1 = $details[0]['no_passengers'];
    if($passenger1!='')
    {
      $passenger = (int)$passenger1;
    }
    ...
    if($details[0]['notes']!='')
    {
      if($details[0]['no_large_cases']==1 && $details[0]['no_cabin_cases']==1){
        $ins = "1 Large Case, 1 Small Case, ".$details[0]['notes'];
        ...
        $ins = "1 Small Case, ".$details[0]['notes'];
      } else if($details[0]['no_large_cases']=="1" && $details[0]['no_cabin_cases']=="0"){
        $ins = "1 Large Case, ".$details[0]['notes'];
      } else if($details[0]['no_large_cases']=="0" && $details[0]['no_cabin_cases']!="1"){
        $ins = $details[0]['no_cabin_cases']."  Small Case, ".$details[0]['notes'];
      } else if($details[0]['no_large_cases']!="1" && $details[0]['no_cabin_cases']=="0"){
        $ins = $details[0]['no_large_cases']." Large Case, ".$details[0]['notes'];
      } else { 
        $ins = $details[0]['no_large_cases']." Large Cases, ".$details[0]['no_cabin_cases']." Small Cases, ".$details[0]['notes'];
      }
    }
    else
    {
      if($details[0]['no_large_cases']==1 && $details[0]['no_cabin_cases']==1){
        $ins = "1 Large Case, 1 Small Case";
      } else if($details[0]['no_large_cases']=="0" && $details[0]['no_cabin_cases']=="0"){
        $ins = "";
      } else if($details[0]['no_large_cases']=="0" && $details[0]['no_cabin_cases']=="1"){
        $ins = "1 Small Case";
        ...
        $ins = $details[0]['no_cabin_cases']."  Small Case";
      } else if($details[0]['no_large_cases']!="1" && $details[0]['no_cabin_cases']=="0"){
        $ins = $details[0]['no_large_cases']." Large Case";
      } else { 
        $ins = $details[0]['no_large_cases']." Large Cases, ".$details[0]['no_cabin_cases']." Small Cases";
      }
    }
    ...
    if($data['body']['booking']['trip_id']!='')
    {
      ...
      try {
        ...
        if ($response['result'] === 'Success') {
          ...
          $data10 = array(
            "to" => array($details[0]['customer_email']=>"Inbound Booking Confirmation"),
            ...
            "subject" => "Inbound Booking Confirmation",
            "html" => "  <html>
            <head>
            <meta charset='UTF-8'>
...
$this->get_include();
$this->load->view($this->viewDir.'inbound/step7',$this->data);
}
...
public function thank_you()
{
  ...
  $this->get_include();
  $this->load->view($this->viewDir.'inbound/thank-you',$this->data);
}
