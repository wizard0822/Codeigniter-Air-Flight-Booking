<?php
    ...
    define('MYSQL_USER', 'skybound_user');
    define('MYSQL_PASS', '{cG-cc;R%y%p');
    define('MYSQL_DB', 'skybound_db');
    ...
    elseif( isset($_POST['new_zone']) ) {
		sql_query('INSERT INTO sb_Zone_tbl SET ID_Sheet="'.intval($_POST['id_sheet']).'",
	...
	// save zone name
    //
    elseif( isset($_POST['save_name']) ) {
		sql_query('UPDATE sb_Zone_tbl SET 
                ID_Zone="'.intval($_POST['id_zone']).'", 
		        Name="'.sql($_POST['save_name']).'" 
		        WHERE ID_ZoneCode="'.intval($_POST['rec_id']).'"');
		echo '{"ok":"ok"}';
		exit();
	}
	//
    ...
    elseif( isset($_POST['save_colour']) ) {
		...
		sql_query('UPDATE sb_Zone_tbl SET Colour="'.hexdec($colour).'" WHERE ID_ZoneCode="'.intval($_POST['rec_id']).'"');
		echo '{"ok":"ok"}';
		exit();
	}
	//
    ...
    elseif( isset($_POST['save_points']) ) {
	    if( isset($_POST['points']) && count($_POST['points']) ) {
		    ...
		    $polygon[] = $polygon[0];
		    sql_query($sql=('UPDATE sb_Zone_tbl SET Zone=PolyFromText("POLYGON(('.implode(',',$polygon).'))") 
    ...
    elseif( isset($_POST['delete_zone']) ) {
		sql_query('DELETE FROM sb_Zone_tbl WHERE ID_ZoneCode="'.intval($_POST['delete_zone']).'"');
		echo '{}';
		exit();
	}
	//
    ...
    elseif( isset($_POST['new_sheet']) ) {
		if( sql_query('INSERT INTO sb_ZoneEditor_tbl SET 
    ...
    elseif( isset($_POST['center_sheet']) ) {
		if( sql_query('UPDATE sb_ZoneEditor_tbl SET 
            SheetName="'.sql($_POST['name']).'",
            CentreLatitude="'.sql($_POST['lat']).'",
		    CentreLongitude="'.sql($_POST['lng']).'",
		    ZoomLevel="'.intval($_POST['zoom']).'"
    ...
    //
    $sheets = array();
	$r = sql_query('SELECT * FROM sb_ZoneEditor_tbl ORDER BY ID_ZoneEditor');
